...
public class RxBus {
            ...
            for (Method method : methods) {
                if (method.isAnnotationPresent(Subscribe.class)) {
                    ...
                    if (parameterType != null && parameterType.length == 1) {
                        ...
                        SubscriberMethod subscriberMethod = new SubscriberMethod(subscriber, method, eventType, threadMode);
    ...
    private void addSubscriber(final SubscriberMethod subscriberMethod) {
        Flowable flowable = toObservable(subscriberMethod.eventType);;
    ...
    private void callEvent(SubscriberMethod method, Object object) {
        ...
        if (methods != null && methods.size() > 0) {
            for (SubscriberMethod subscriberMethod : methods) {
                if (method.subscriber.equals(subscriberMethod.subscriber) && method.method.equals(subscriberMethod.method)) {
